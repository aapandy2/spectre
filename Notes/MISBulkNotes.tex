...
In particular, the conserved currents are
\begin{align}
...
\textbf{TODO: rederive everything in this section in Mathematica.}
This should be done by coding up the EOM and solving for the components of $\nabla_a \Psi$, where $\Psi$ is defined below.

Fortunately we can rewrite the MIS-bulk equations in a useful way.
Specifically, they can be written as a first-order symmetric hyperbolic (FOSH) system, whereby they take the form
\begin{equation} \label{eq:MIS_bulk_FOSH_form}
0 = M^a(\Psi) \nabla_a \Psi + N(\Psi) \Psi
\end{equation}
where $N^\mu$ are matrices, $N$ is a vector, and $\Psi = (\epsilon, u^b, n, \Pi)^T$ contains all of the state variables.
Notice that in (\ref{eq:MIS_bulk_FOSH_form}) each equation only includes one covariant derivative term, so in particular the $\Pi$ equation will be:
\begin{equation}
0 = M^t(\Psi) \partial_t \Psi + M^i(\Psi) \partial_i \Psi + N(\Psi) \Psi
\end{equation}
and will be free of time derivatives of the other variables.
We should be able to use this equation combined with the conservation laws in the typical (Valencia) form to evolve the system.
