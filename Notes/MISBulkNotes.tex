...
\begin{equation}
\begin{aligned}
\partial_t W^{-2} &= \partial_t ( 1 - \gamma_{ij} v^i v^j ) \\
\partial_t W &= \frac{1}{2} W^3 \partial_t ( \gamma_{ij} v^i v^j ) \\
&= \frac{1}{2} W^3 \Big( v^i v^j \partial_t \gamma_{ij} + 2 v_j \partial_t v^j \Big) \\
\end{aligned}
\end{equation}
in agreement with \cite{Servignat22} equation 29.
We still need to eliminate the time derivatives on the RHS, the first one which can be handled using the evolution equation for the spatial metric (\cite{BaumgarteShapiro} equation 2.134):
\begin{equation}
\partial_t \gamma_{ij} = -2 \alpha K_{ij} + D_i \beta_j + D_j \beta_i.
\end{equation}

To obtain the other term, we have to use the conservation laws.

\textbf{DO THE STUFF BELOW IN STANDARD MATHEMATICA}

%We will begin with the second term in parentheses, which we can get from expanding out the momentum conservation law:
%\begin{equation}
%\begin{aligned}
%0 &= \partial_t \big[ \sqrt{\gamma} \rho h W^2 v_j \big] + \partial_i \big[ \sqrt{\gamma} (\alpha S^{i}_{~j} - \beta^{i} S_j) \big] - \frac{1}{2} \sqrt{|g|} T^{ac} \partial_{j} g_{ac} \\
%\partial_t v_j &= - \frac{1}{\sqrt{\gamma} \rho h W^2}\Big[ \rho h W^2 v_j \partial_t \sqrt{\gamma} + \sqrt{\gamma} W^2 v_j \partial_t e + \sqrt{\gamma} W^2 v_j \partial_t p + 2 W \sqrt{\gamma} \rho h v_j \partial_t W \\
%&+ \partial_i \big[ \sqrt{\gamma} (\alpha S^{i}_{~j} - \beta^{i} S_j) \big] - \frac{1}{2} \sqrt{|g|} T^{ac} \partial_{j} g_{ac} \Big]. \\
%\end{aligned}
%\end{equation}
%Notice that this still has time derivative terms we don't already have, namely $\partial_t e, \partial_t p$.
%
%We will handle the second term first, as it is the most intensive---it turns out we will need $\partial_t \rho, \partial_t e$, and thus will need to expand out all three of the Valencia-form relativistic Euler equations.
%Doing so for the baryon current conservation law gives us
%\begin{equation}
%\begin{aligned}
%0 
%%&= \partial_t (\sqrt{\gamma} D) + \partial_i \big[ \sqrt{\gamma} D (\alpha v^i - \beta^i) \big] \\
%&= \partial_t (\sqrt{\gamma} \rho W) + \partial_i \big[ \sqrt{\gamma} D (\alpha v^i - \beta^i) \big] \\
%\partial_t \rho &= -\frac{1}{\sqrt{\gamma} W} \Big( \rho W \partial_t \sqrt{\gamma} + \sqrt{\gamma} \rho \partial_t W + \partial_i \big[ \sqrt{\gamma} D (\alpha v^i - \beta^i) \big] \Big) \\
%\end{aligned}
%\end{equation}
%the energy equation yields
%\begin{equation}
%\begin{aligned}
%0 &= \partial_t (\sqrt{\gamma} [\rho h W^2 - p]) + \partial_i (\sqrt{\gamma} [\alpha S^i - \beta^i E]) + \sqrt{|g|} T^{ab} \nabla_a n_b \\
%&= \sqrt{\gamma} W^2 \Big( 1 + v_i v^i \frac{\partial p}{\partial e} \Big) \partial_t e + \sqrt{\gamma} W^2 v_i v^i \frac{\partial p}{\partial \rho} \partial_t \rho + 2 W \sqrt{\gamma} [e+p] \partial_t W + [\rho h W^2 - p] \partial_t (\sqrt{\gamma}) \\
%&+ \partial_i (\sqrt{\gamma} [\alpha S^i - \beta^i E]) + \sqrt{|g|} T^{ab} \nabla_a n_b \\
%\end{aligned}
%\end{equation}
...
%\begin{aligned}
%0 &= \partial_t \big[ \sqrt{\gamma} \rho h W^2 v_j \big] + \partial_i \big[ \sqrt{\gamma} (\alpha S^{i}_{~j} - \beta^{i} S_j) \big] - \frac{1}{2} \sqrt{|g|} T^{ac} \partial_{j} g_{ac} \\
%&= \rho h W^2 v_j \partial_t \big[ \sqrt{\gamma} \big] + \sqrt{\gamma} W^2 v_j \partial_t \big[ e+p \big] + \sqrt{\gamma} \rho h v_j \partial_t \big[ W^2 \big] + \sqrt{\gamma} \rho h W^2 \partial_t \big[ v_j \big] \\
%&+ \partial_i \big[ \sqrt{\gamma} (\alpha S^{i}_{~j} - \beta^{i} S_j) \big] - \frac{1}{2} \sqrt{|g|} T^{ac} \partial_{j} g_{ac} \\
%\end{aligned}
%\end{equation}
